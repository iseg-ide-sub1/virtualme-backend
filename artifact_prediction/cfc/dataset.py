...
# 尝试加载一个小的预训练模型
wv = api.load('glove-wiki-gigaword-50')  # 一个较小的 GloVe 模型
...
def load_train_data_raw(train_data_folder):
...
def train_data_raw_2_dataset(train_data_list, pred_len=1, max_seq_len=500):
    times = []
    event_types = []
    feedbacks = []
    artifact_embeds = []
    ...
    for i, event in enumerate(train_data_list):
        ...
        event_type_embed = torch.tensor(wv[event_type] if event_type in wv else [0.0] * 50)
        ...
        for j in range(pred_len):
            ...
            if next_event['artifactEmbed'] is None:
                next_artifact_embed = torch.zeros(model_params['artifact_embedding_dim'])
            else:
                next_artifact_embed = torch.tensor(next_event['artifactEmbed'])
            labels.append(next_artifact_embed)
    ...
    while offset + max_seq_len < seq_length:
        ...
        seqs.append(
            (
                times[idx] - first_tp, event_types[idx], feedbacks[idx], artifact_embeds[idx], candidate_embeds_list[idx],
                labels_list[idx])
        )
        offset += slide

    event_dataset = EventDataset(seqs)
    return event_dataset


if __name__ == '__main__':
    max_seq_len = model_params['max_seq_len']
    train_data_list = load_train_data_raw('../../dataset_raw')
    event_dataset = train_data_raw_2_dataset(train_data_list, pred_len=1, max_seq_len=max_seq_len)
    print(event_dataset)
